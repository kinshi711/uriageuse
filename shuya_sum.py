...
def osi_sum():
    o_sum = sum([sheet1.range('B{}'.format(n1)).options(empty=0).value * sheet1.range('C{}'.format(n1)).options(
...
def water_sum():
    w_sum = sum([sheet1.range('B{}'.format(n2)).options(empty=0).value * sheet1.range('C{}'.format(n2)).options(
        empty=0).value for n2 in range(5, 10, 1)])
    water = int(w_sum)
    return water

def ice_sum():
    i_sum = sum([sheet1.range('B{}'.format(n3)).options(empty=0).value * sheet1.range('C{}'.format(n3)).options(
