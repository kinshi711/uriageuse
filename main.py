...
month = int(w_book.sheets['売り上げ記入用'].range('H2').options(empty='選択肢してください').value)  # 書き込まれた月の参照
day = int(w_book.sheets["売り上げ記入用"].range('J2').options(empty='選択して下さい').value)  # 書き込まれた日の参照
s_dir = os.path.join(dir, '保存先')  # 保存用フォルダーの参照
year = int(w_book.sheets['売り上げ記入用'].range('F2').options(empty='選択肢してください').value)  # 書き込まれた年度を参照
